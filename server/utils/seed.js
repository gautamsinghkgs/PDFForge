...
// ✅ FIX: Explicitly point dotenv to server/.env
// Previously `require('dotenv').config()` looked for .env in process.cwd()
// (i.e. the project root when you run `node server/utils/seed.js` from root).
// The .env file lives inside server/, so we must give the explicit path.
...
async function seed() {
  try {
    if (!process.env.MONGO_URI) {
      throw new Error(
        'MONGO_URI is not defined. Make sure server/.env file exists and contains MONGO_URI.'
      );
    }
